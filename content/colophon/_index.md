+++
insert_anchor_links = "left"
title = "Colophon"
+++

<aside>
    90% human-led, 10% AI assisted
</aside>

Thanks to a stellar theme, I was able to do the heavy lifting (writing, decision-making, implementation), and only sed AI for troubleshooting and guidance.

### Site
* Theme: [Duckquill](https://duckquill.daudix.one) by [David Lapshin](https://daudix.one)
* Typography
  * Fraunces
  * Quicksand
  * Spot Mono

### Tools

* Claude 4.5 Haiku (config, css) 
* [Zed](https://zed.dev) code editor
* [Ghostty](https://ghostty.org) terminal emulator

### Media
* Homepage ASCII from [ACIIart](https://asciiart.website/art/866)
* About Image from [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Table-banquette_by_Annie_Tribel_1968_-_Design_Museum_Brussels_Belgium.jpg)
